...
type MessageSource struct {
	Path        string `json:"path"`         // path on disk OR path inside zip
	InZip       bool   `json:"in_zip"`       // whether it's inside a zip
	ZipPath     string `json:"zip_path"`     // path to the zip file itself
	DisplayName string `json:"display_name"` // name shown in the UI list or logs
}
