...
func TestMessageSourceJSONSerializationContract(t *testing.T) {
	src := MessageSource{
		Path:        "test.msg",
		InZip:       true,
		ZipPath:     "archive.zip",
		DisplayName: "test.msg (archive.zip)",
	}
	...
	expectedKeys := []string{"path", "in_zip", "zip_path", "display_name"}
	expectedSet := make(map[string]bool)
	for _, k := range expectedKeys {
		expectedSet[k] = true
	}

	if len(rawMap) != len(expectedSet) {
		t.Errorf("expected %d JSON keys matching expected set, got %d (%v)", len(expectedSet), len(rawMap), rawMap)
	}

	for k := range rawMap {
		if !expectedSet[k] {
			t.Errorf("unexpected JSON key %q found in marshalled MessageSource", k)
		}
	}
	for _, k := range expectedKeys {
		if _, ok := rawMap[k]; !ok {
			t.Errorf("missing expected JSON key %q in marshalled MessageSource", k)
		}
	}
}
