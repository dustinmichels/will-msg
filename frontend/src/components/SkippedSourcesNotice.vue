...
<template>
  ...
  >
    <div class="flex items-center justify-between">
      <div class="flex items-center gap-2">
        ...
        <span class="font-semibold">
          {{ parseStore.skipped.length }} source file{{ parseStore.skipped.length === 1 ? '' : 's' }} could not be parsed
        </span>
      </div>
    ...
    <div v-if="isExpanded" class="mt-2.5 pt-2 border-t border-amber-200 dark:border-amber-800/60 max-h-48 overflow-y-auto space-y-1.5 font-mono">
      <div
      ...
      >
        <span class="font-semibold text-amber-950 dark:text-amber-100 break-all">{{ item.display_name }}:</span>
