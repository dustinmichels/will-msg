...
<template>
  <div class="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-850 p-4 space-y-3 shadow-xs select-none">
    <!-- Header -->
    <div>
      <h3 class="text-xs font-bold text-slate-900 dark:text-slate-100 flex items-center gap-2">
        <span>Live Rule Tester / Sandbox</span>
        <span v-if="isLoading" class="text-[10px] font-normal text-truck-600 dark:text-truck-400 animate-pulse">Evaluating…</span>
      </h3>
      <p class="text-[11px] text-slate-500 dark:text-slate-400 italic">
        Type or paste any raw message line to verify address splitting, matching rule precedence, and assigned label in real time:
      </p>
    </div>
    ...
    <div class="flex flex-wrap items-center gap-2 text-xs">
      <span class="text-[11px] font-bold text-slate-500 dark:text-slate-400 mr-1">Quick Samples:</span>
      <button
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2.5 pt-1">
      <!-- 1. Detected Address -->
      <div class="p-2.5 rounded-lg bg-slate-50 dark:bg-slate-900/60 border border-slate-200/80 dark:border-slate-800">
        <div class="text-[10px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-0.5">
          Detected Address:
        </div>
        <div class="text-xs font-bold font-mono text-slate-800 dark:text-slate-200 truncate" :title="result?.address">
      ...
      <div class="p-2.5 rounded-lg bg-slate-50 dark:bg-slate-900/60 border border-slate-200/80 dark:border-slate-800">
        ...
        <div class="text-xs font-bold font-mono text-slate-800 dark:text-slate-200 truncate" :title="result?.status">
      ...
      <div class="p-2.5 rounded-lg bg-slate-50 dark:bg-slate-900/60 border border-slate-200/80 dark:border-slate-800">
        ...
        <div class="text-xs font-bold font-mono text-truck-600 dark:text-truck-400 truncate" :title="result?.label">
      ...
      <div class="p-2.5 rounded-lg bg-slate-50 dark:bg-slate-900/60 border border-slate-200/80 dark:border-slate-800">
        ...
        <div class="text-xs font-bold text-slate-800 dark:text-slate-200 truncate" :title="formatMatchedRule()">
          {{ formatMatchedRule() }}
        </div>
      </div>
