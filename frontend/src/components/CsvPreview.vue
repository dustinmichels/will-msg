    ...
    <div class="flex-1 overflow-auto border border-slate-200 dark:border-slate-800 rounded-lg">
      <table class="w-full text-left text-xs border-collapse">
        ...
        >
          ...
          >
            <td
              class="px-3 py-1.5 border-r border-slate-200 dark:border-slate-800 truncate font-mono text-[11px]"
              :title="rec.source_file"
            >
              {{ rec.source_file }}
            </td>
            <td
              class="px-3 py-1.5 border-r border-slate-200 dark:border-slate-800 truncate"
              :title="rec.subject"
            >
              {{ rec.subject }}
            </td>
            <td
              class="px-3 py-1.5 border-r border-slate-200 dark:border-slate-800 whitespace-nowrap tabular-nums font-mono text-[11px]"
            >
              {{ rec.message_date }}
            </td>
            <td
            ...
            <td
              class="px-3 py-1.5 border-r border-slate-200 dark:border-slate-800 truncate"
              :title="rec.dispatcher"
            >
              {{ rec.dispatcher }}
            </td>
            <td
              class="px-3 py-1.5 border-r border-slate-200 dark:border-slate-800 text-center tabular-nums font-mono text-[11px]"
            >
              {{ rec.row_in_message }}
            </td>
            <td
            ...
            <td
              class="px-3 py-1.5 border-r border-slate-200 dark:border-slate-800 truncate font-medium"
              :title="rec.location"
            >
              {{ rec.location }}
            </td>
            ...
            <td
              class="px-3 py-1.5 border-r border-slate-200 dark:border-slate-800 whitespace-nowrap font-mono text-[11px] font-semibold text-truck-600 dark:text-truck-400"
            >
              {{ rec.label }}
            </td>
          ...
          <tr v-if="parseStore.records.length === 0">
            <td
              ...
              class="px-6 py-16 text-center text-slate-400 dark:text-slate-500"
            >
              <div class="flex flex-col items-center justify-center gap-2">
                ...
                <span class="text-sm italic">
                  {{
                    parseStore.isParsing
                      ? "Parsing records…"
                      : 'No records parsed yet. Click "Run Parser" to extract rows.'
                  }}
                </span>
              </div>
            </td>
