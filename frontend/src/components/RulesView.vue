...
async function handleResetDefaults() {
  ...
  if (shouldReset) {
    try {
      ...
      toast.info(
        "Classification rules and labels have been reset to built-in defaults.",
        "Reset Defaults",
      );
    } catch (err: unknown) {
...
async function handleImport() {
  ...
  try {
    ...
    if (res && !res.cancelled && res.config) {
      toast.success(
        `Imported ${res.config.rules?.length || 0} rules successfully.`,
        "Rules Imported",
      );
    }
  } catch (err: unknown) {
...
async function handleSaveAndApply() {
  ...
  try {
    ...
    if (success) {
      toast.success(
        "Classification rules have been saved and applied to the parser.",
        "Rules Saved & Applied",
      );
...
<template>
  <div
    class="flex-1 flex flex-col min-h-0 bg-slate-50 dark:bg-slate-900 overflow-hidden select-none"
  >
    <!-- Header Banner -->
    <div
      class="bg-truck-500 text-white px-4 py-2.5 flex items-center justify-between shadow-xs shrink-0"
    >
      <div class="flex items-center gap-2">
        ...
        <h2 class="text-base font-bold text-white tracking-tight">Rules & Labels Manager</h2>
      </div>
    ...
    <div
      class="p-3 bg-white dark:bg-slate-850 border-b border-slate-200 dark:border-slate-800 shrink-0 space-y-2.5"
    >
      <!-- Toolbar Row -->
      ...
      <!-- Settings Bar Row -->
      <div
        class="flex flex-wrap items-center justify-between gap-3 pt-2 border-t border-slate-100 dark:border-slate-800 text-xs"
      >
        <div class="flex items-center gap-2">
          <input
            id="heuristics-check"
          ...
          />
          <label
            for="heuristics-check"
            class="text-slate-700 dark:text-slate-300 font-medium cursor-pointer"
          >
    ...
    <!-- Bottom Action Bar -->
    <div
      class="px-4 py-3 bg-white dark:bg-slate-850 border-t border-slate-200 dark:border-slate-800 flex items-center justify-between shrink-0 shadow-xs"
    >
