...
describe('Parse Store', () => {
  ...
  it('scans sources successfully and updates state', async () => {
    const mockSources: scanner.MessageSource[] = [
      scanner.MessageSource.createFrom({ path: '/path/to/file1.msg', in_zip: false, zip_path: '', display_name: 'file1.msg' }),
    ]
    ...
    const globalObj = globalThis as unknown as {
      window: { go: { appservice: { Service: { ScanSources: (paths: string[]) => Promise<appservice.ScanResult> } } } }
    }
  ...
  it('suppresses stale parse Promise results after reset', async () => {
    ...
    const globalObj = globalThis as unknown as {
      window: { go: { appservice: { Service: { Parse: () => Promise<appservice.ParseResult>; ClearParse: () => Promise<void> } } } }
    }
    globalObj.window.go.appservice.Service.Parse = () => parsePromise
    globalObj.window.go.appservice.Service.ClearParse = async () => {}

    const store = useParseStore()
    store.sources = [scanner.MessageSource.createFrom({ path: '/test.msg', in_zip: false, zip_path: '', display_name: 'test.msg' })]
    ...
    resolveParse(appservice.ParseResult.createFrom({
  ...
  it('scans multiple dropped or selected files in a single ScanSources call', async () => {
    ...
    const mockScanResult: appservice.ScanResult = appservice.ScanResult.createFrom({
      ...
      sources: [
        scanner.MessageSource.createFrom({ path: '/dir/sub/f1.msg', in_zip: false, zip_path: '', display_name: 'f1.msg' }),
        scanner.MessageSource.createFrom({ path: '/file1.msg', in_zip: false, zip_path: '', display_name: 'file1.msg' }),
        scanner.MessageSource.createFrom({ path: 'inside.msg', in_zip: true, zip_path: '/archive.zip', display_name: 'inside.msg (in archive.zip)' }),
      ],
    })
  ...
  it('handles cancelled saveAs without error or state mutation', async () => {
    ...
    globalObj.window.go.appservice.Service.SaveAs = async () => appservice.SavedFile.createFrom({ cancelled: true })
