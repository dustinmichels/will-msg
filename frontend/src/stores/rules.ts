...
export const useRulesStore = defineStore('rules', {
  ...
  getters: {
    selectedRule: (state): config.ClassificationRule | null => {
      if (!state.workingConfig || state.selectedIndex < 0 || state.selectedIndex >= state.workingConfig.rules.length) {
        return null
      }
  ...
  actions: {
    ...
    async addRule(rule: Partial<config.ClassificationRule> & { pattern: string; label: string }): Promise<void> {
      if (this.isSaving) {
        throw new Error('Cannot modify rules while save is in progress')
      }
    ...
    async moveRule(index: number, direction: 'up' | 'down'): Promise<boolean> {
      ...
      if (index < 0 || index >= this.workingConfig.rules.length || targetIndex < 0 || targetIndex >= this.workingConfig.rules.length) {
        return false
      }
      await this.ensureDirty()
    ...
    async updateSettings(settings: { enable_heuristics?: boolean; default_label?: string }): Promise<void> {
      if (this.isSaving) {
        throw new Error('Cannot modify rules while save is in progress')
      }
      if (!this.workingConfig) {
