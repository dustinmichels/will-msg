...
describe('Rules Store', () => {
  ...
  const sampleConfig: config.RuleConfig = config.RuleConfig.createFrom({
    ...
    default_label: 'other',
    labels: [
      config.LabelDefinition.createFrom({ key: 'msw_not_out', display_name: 'Trash Not Out', metric: 'trash' }),
      config.LabelDefinition.createFrom({ key: 'recyc_not_out', display_name: 'Recycle Not Out', metric: 'recycling' }),
      config.LabelDefinition.createFrom({ key: 'other', display_name: 'Other', metric: 'none' }),
    ],
    rules: [
      config.ClassificationRule.createFrom({ id: 'rule_1', pattern: 'TRASH NOT OUT', type: 'substring', label: 'msw_not_out', enabled: true }),
      config.ClassificationRule.createFrom({ id: 'rule_2', pattern: 'RCY NOT OUT', type: 'substring', label: 'recyc_not_out', enabled: true }),
    ],
  })
  ...
  it('enforces dirty-mirror ordering: calls SetRulesDirty(true) before mutating local state', async () => {
    let order: string[] = []
    const globalObj = globalThis as unknown as {
      window: { go: { appservice: { Service: { SetRulesDirty: (dirty: boolean) => Promise<void> } } } }
    }
  ...
  it('rejects every config mutation while isSaving is true', async () => {
    ...
    expect(store.addRule({ pattern: 'TEST', label: 'other' })).rejects.toThrow('Cannot modify rules while save is in progress')
    expect(store.updateRule(0, { pattern: 'TEST' })).rejects.toThrow('Cannot modify rules while save is in progress')
    expect(store.deleteRule(0)).rejects.toThrow('Cannot modify rules while save is in progress')
    expect(store.moveRule(0, 'down')).rejects.toThrow('Cannot modify rules while save is in progress')
    ...
    expect(store.importConfig(sampleConfig)).rejects.toThrow('Cannot modify rules while save is in progress')
    expect(store.updateSettings({ enable_heuristics: false })).rejects.toThrow('Cannot modify rules while save is in progress')
    expect(store.addOrUpdateLabel(config.LabelDefinition.createFrom({ key: 'new', display_name: 'New', metric: 'none' }))).rejects.toThrow('Cannot modify rules while save is in progress')
    expect(store.deleteLabel('other')).rejects.toThrow('Cannot modify rules while save is in progress')
  })
  ...
  it('handles successful importRulesFromFile by updating workingConfig with deep-clone isolation and marking dirty', async () => {
    const importedCfg = config.RuleConfig.createFrom({
      ...
      labels: [
        config.LabelDefinition.createFrom({ key: 'custom_default', name: 'Custom Default', metric: 'none' }),
      ],
      rules: [
  ...
  it('handles cancelled exportRulesToFile returning cancelled result without error', async () => {
    ...
    globalObj.window.go.appservice.Service.ExportRules = async () => appservice.SavedFile.createFrom({ cancelled: true })
